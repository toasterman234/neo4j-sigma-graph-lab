...
async def get_personal_schema_graph(limit: int = 180) -> dict:
    """Build the app-schema view from read-only Vault content in Neo4j.

    The graph stores imported Vault material as Chunk nodes with source URIs,
    plus semantic entities. This endpoint projects those existing records into
    the app's schema vocabulary without writing anything back to Neo4j.
    """
    safe_limit = max(20, min(limit, 300))
    rows = await execute_cypher(
        f"""
        MATCH (c:Chunk)
        WHERE c.`metadata_x-amz-bedrock-kb-source-uri` CONTAINS '/ben-vault/'
        WITH c, c.`metadata_x-amz-bedrock-kb-source-uri` AS uri
        RETURN c, uri
        ORDER BY uri
        LIMIT {safe_limit}
        """,
    ...
    semantic_rows = await execute_cypher(
        """
        MATCH (a)-[r]->(b)
        WHERE (a:SemanticEntity OR a:Project OR a:Service OR a:System OR a:Platform
               OR a:Decision OR a:Task OR a:Process OR a:SyncOption OR a:DecisionState)
          AND (b:SemanticEntity OR b:Project OR b:Service OR b:System OR b:Platform
               OR b:Decision OR b:Task OR b:Process OR b:SyncOption OR b:DecisionState)
        RETURN a, r, b
        LIMIT 120
        """,
        collect=False,
    ...
    def category(path: str) -> str:
        lower = path.lower()
        if "/things" in lower or "things-" in lower or "things_" in lower:
            return "Thing"
        if "/workspace/projects/" in lower or "/items/project" in lower:
            return "Project"
        if "/items/work" in lower or "/control/requests/" in lower:
            return "Task"
        if "decision" in lower or "/decisions/" in lower:
            return "Decision"
        if "/system/" in lower:
            return "System"
        if "/explore" in lower or "explore -" in lower:
            return "Explore"
        if "/work/" in lower or "work -" in lower:
            return "Work"
        return "Note"

    nodes: list[dict] = []
    relationships: list[dict] = []
    categories = ["Vault", "Note", "Thing", "Project", "Task", "Decision", "System", "Explore", "Work"]
    category_ids = {name: f"app-schema:{name.lower()}" for name in categories}
    for name, element_id in category_ids.items():
        nodes.append({"elementId": element_id, "labels": [name], "name": name, "schemaNode": True})

    seen = set(category_ids.values())
    for row in rows:
        raw = row.get("c")
        ...
        if node_id in seen:
            continue
        path = uri.split("/ben-vault/", 1)[-1].replace("%20", " ")
        kind = category(path)
        node["labels"] = [kind, "VaultNote"]
        node["name"] = path.rsplit("/", 1)[-1].removesuffix(".md")
        node["source_path"] = path
        node["schemaNode"] = False
        nodes.append(node)
        seen.add(node_id)
        relationships.append({
            "elementId": f"app-schema:contains:{node_id}",
            "type": "CONTAINS",
            "startNodeElementId": category_ids["Vault"],
            "endNodeElementId": node_id,
        })
        relationships.append({
            "elementId": f"app-schema:kind:{node_id}",
            "type": "INSTANCE_OF",
            "startNodeElementId": category_ids[kind],
            "endNodeElementId": node_id,
        })
    ...
    for row in semantic_rows:
        for key in ("a", "b"):
            raw = row.get(key)
            if isinstance(raw, dict) and raw.get("elementId"):
                node_id = str(raw["elementId"])
                if node_id not in seen:
                    node = dict(raw)
                    node["name"] = node.get("name") or node.get("neptune_id") or node_id[:12]
                    nodes.append(node)
                    seen.add(node_id)
        raw_rel = row.get("r")
        if isinstance(raw_rel, dict) and raw_rel.get("elementId"):
            relationships.append(raw_rel)

    return {"nodes": nodes, "relationships": relationships, "source": "ben-vault", "limit": safe_limit}


async def get_existing_graph_overview(limit: int = 120) -> dict:
    """Get a bounded, read-only overview of the existing Neo4j graph.

    This intentionally avoids domain filtering so the generated UI can inspect
    an already-populated graph whose nodes predate this scaffold. It only reads
    a bounded sample of connected nodes and relationships; node expansion stays
    on-demand through ``expand_node``.
    """
    safe_limit = max(1, min(limit, 500))
    labels = await execute_cypher(
        """
        MATCH (n)
    ...
    relationship_types = await execute_cypher(
        """
        MATCH ()-[r]->()
