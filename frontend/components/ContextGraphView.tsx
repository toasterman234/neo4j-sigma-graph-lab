...
export function ContextGraphView({ externalGraphData, onAskAbout }: ContextGraphViewProps) {
  ...
  const [isExistingGraph, setIsExistingGraph] = useState(false);
  const [isPersonalSchemaGraph, setIsPersonalSchemaGraph] = useState(false);
  ...
  async function loadExistingGraph() {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE}/existing-graph?limit=160`, { signal: AbortSignal.timeout(15000) });
      if (!res.ok) throw new Error(`Existing graph request failed: ${res.status}`);
      const data = await res.json();
      const parsed = extractNodesAndRels(data.results || []);
      setGraphData(parsed);
      setIsSchemaView(false);
      setIsExistingGraph(true);
      setIsPersonalSchemaGraph(false);
      setExpandedNodeIds(new Set());
      setSelectedElement(null);
      setSelectedNodeId(null);
      setSelectedRelId(null);
    } catch {
      setError("Unable to load the existing Neo4j graph.");
    } finally {
      setLoading(false);
    }
  }

  async function loadPersonalSchemaGraph() {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`${API_BASE}/personal-schema-graph?limit=180`, { signal: AbortSignal.timeout(20000) });
      if (!res.ok) throw new Error(`Personal schema graph request failed: ${res.status}`);
      const data = await res.json();
      const parsed = extractNodesAndRels([{ nodes: data.nodes || [], relationships: data.relationships || [] }]);
      setGraphData(parsed);
      setIsSchemaView(true);
      setIsExistingGraph(false);
      setIsPersonalSchemaGraph(true);
      setExpandedNodeIds(new Set());
      setSelectedElement(null);
      setSelectedNodeId(null);
      setSelectedRelId(null);
    } catch {
      setError("Unable to load Vault content into the app schema.");
    } finally {
      setLoading(false);
    }
  }
        ...
        <Flex w="100%" align="center" justify="space-between" gap={3}>
          <Box minW={0}>
            ...
            <Text fontSize="xs" color="gray.500">
              {isPersonalSchemaGraph
                ? "App schema populated from your Vault, Things, and personal graph"
                : isSchemaView
                  ? "Schema view — double-click a label to explore"
                  : isExistingGraph
                    ? "Existing ZimaOS Neo4j graph — read-only overview"
                    : "Personal Knowledge entity relationships"}
            </Text>
          </Box>
          <HStack gap={2} flexShrink={0}>
            <Button size="sm" colorPalette="blue" variant={isPersonalSchemaGraph ? "solid" : "outline"} onClick={loadPersonalSchemaGraph}>
              Personal Vault
            </Button>
            <Button size="sm" variant={isSchemaView && !isPersonalSchemaGraph ? "solid" : "outline"} onClick={loadSchema}>
              App Schema
            </Button>
          </HStack>
        </Flex>
        ...
        {nvlData.nodes.length > 0 ? (
          ...
          <Flex h="100%" align="center" justify="center" direction="column" gap={4} p={8}>
            ...
            <Text color="gray.500" fontSize="sm" textAlign="center" maxW="300px">
              Choose “My Graph” for your existing ZimaOS Neo4j data, or use the app schema to explore the generated demo model.
            </Text>
          </Flex>
        )}
      </Box>
