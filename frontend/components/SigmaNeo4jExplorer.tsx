...
export function SigmaNeo4jExplorer() {
  ...
  return <div className="sigma-explorer">
    ...
    <div className="sigma-controls">
      ...
      <div className="sigma-selector-group"><span>Question catalog</span><select aria-label="Jev catalog question" value={questionId} onChange={(e) => { setQuestionId(e.target.value); setReasoning(null); setActiveSavedId(null); }}>{manualQuestions.map((question) => <option key={question.id} value={question.id}>{question.group} · {question.title} · {question.mode === "item" ? "needs a selected node" : "needs a search query"}</option>)}</select><span className="sigma-muted">{selectedQuestion.mode === "item" ? "needs a selected node" : "needs a search query"} · v{selectedQuestion.version} · {selectedQuestion.description}</span><input aria-label="Optional Jev context note" value={reasonQuestion} onChange={(e) => setReasonQuestion(e.target.value)} placeholder="Optional extra context for this run…" />{selectedQuestion.mode === "item" && !selected && <small className="sigma-muted">Select a document/source or graph object to run this item question.</small>}{selectedQuestion.mode === "graph" && !search.trim() && <small className="sigma-muted">Enter a bounded graph search query to run this relationship question.</small>}</div>
