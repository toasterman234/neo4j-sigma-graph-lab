...
test.describe("Personal Knowledge Context Graph", () => {
  ...
  test("page loads with header and chat panel", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Personal Knowledge/i })).toBeVisible();
  ...
  test("backend health check returns ok or degraded", async ({ request }) => {
    ...
    expect(body.domain).toBe("personal-knowledge");
  });
  ...
  test("demo prompt: Knowledge Exploration — sends and gets response", async ({ page }) => {
    test.setTimeout(CHAT_TIMEOUT);

    // Type the prompt
    const input = page.getByPlaceholder(/ask about/i);
    await input.fill("What notes have I written about machine learning this month?");
    await page.getByRole("button", { name: /send/i }).click();

    // Should show user message
    await expect(page.getByText("What notes have I written about machine learning this month?").first()).toBeVisible();
  ...
  test("demo prompt: People & Projects — sends and gets response", async ({ page }) => {
    test.setTimeout(CHAT_TIMEOUT);

    // Type the prompt
    const input = page.getByPlaceholder(/ask about/i);
    await input.fill("Which contacts have I not been in touch with for over 3 months?");
    await page.getByRole("button", { name: /send/i }).click();

    // Should show user message
    await expect(page.getByText("Which contacts have I not been in touch with for over 3 months?").first()).toBeVisible();
  ...
  test("demo prompt: Reflection & Patterns — sends and gets response", async ({ page }) => {
    test.setTimeout(CHAT_TIMEOUT);

    // Type the prompt
    const input = page.getByPlaceholder(/ask about/i);
    await input.fill("What topics appear most frequently in my journal entries?");
    await page.getByRole("button", { name: /send/i }).click();

    // Should show user message
    await expect(page.getByText("What topics appear most frequently in my journal entries?").first()).toBeVisible();
  ...
  test("tool calls show timeline with status indicators", async ({ page }) => {
    ...
    const toolBadge = page.locator("[data-scope='badge']").filter({ hasText: /execute_cypher|get_schema|search_notes/ });
  ...
  test("API: Knowledge Exploration prompt 1 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "What notes have I written about machine learning this month?" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Knowledge Exploration prompt 2 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Show me all topics connected to my graph databases research" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Knowledge Exploration prompt 3 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Find bookmarks I saved but haven\u0027t read yet about distributed systems" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: People & Projects prompt 1 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Which contacts have I not been in touch with for over 3 months?" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: People & Projects prompt 2 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Show me all notes and updates related to my side project" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: People & Projects prompt 3 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Who do I know that works in the AI space?" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Reflection & Patterns prompt 1 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "What topics appear most frequently in my journal entries?" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Reflection & Patterns prompt 2 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "Show me the connections between my reading and my projects" },
    });
    expect(res.ok()).toBeTruthy();
  ...
  test("API: Reflection & Patterns prompt 3 returns quality response", async ({ request }) => {
    test.setTimeout(CHAT_TIMEOUT);

    const res = await request.post(`${API_URL}/api/chat`, {
      data: { message: "What were my most productive weeks and what was I working on?" },
    });
    expect(res.ok()).toBeTruthy();
